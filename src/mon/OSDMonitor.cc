                                         ...
                                         stringstream& ss)
{
  ...
  if (var == "size") {
    ...
    if (n == 1) {
      ...
      if (!g_conf()->mon_allow_pool_size_one) {
	ss << "configuring pool size as 1 is disabled by default.";
	return -EPERM;
      }
      if (!sure) {
	ss << "WARNING: setting pool size 1 could lead to data loss without recovery. If you are *ABSOLUTELY CERTAIN* that is what you want, pass the flag --yes-i-really-really-mean-it.";
	return -EPERM;
      }
    }
